...
> Status: Active
>
> Historical note: the older text-based `TOOL_CALL ...` protocol is deprecated and must not be used for new implementations.

## 1. 目标
本文档定义 AgentJax 的下一阶段 tool calling 正式协议。

要解决的不是“把已废弃的 `TOOL_CALL {...}` 文本协议换个写法”。
而是：
...
- 不再依赖 daemon 主逻辑直接解析已废弃的 `TOOL_CALL ...` 文本
