...
mod tests {
    ...
    use super::{
        manager, ShellExecToolPlugin, ShellSessionCloseToolPlugin, ShellSessionExecToolPlugin,
        ShellSessionInterruptToolPlugin, ShellSessionOpenToolPlugin, ShellSessionResizeToolPlugin,
    };
    use crate::{
    ...
    async fn shell_exec_runs_one_shot_command() {
        let output = ShellExecToolPlugin
            ...
            .await
            .unwrap();
        assert_eq!(output.metadata["status"], "completed");
        assert_eq!(output.metadata["stdout"], "hello\n");
    }

    #[tokio::test]
    async fn shell_session_preserves_export_and_cwd() {
        let root = std::env::temp_dir().join(format!(
            "agentjax-shell-session-{}",
        ...
        let read = poll_until_idle(&session_id).await;
        ...
        assert_eq!(read["cwd"], nested.display().to_string());
    ...
    async fn shell_session_interrupt_stops_running_command() {
        ...
        let interrupted = ShellSessionInterruptToolPlugin
            ...
            .await
            .unwrap();
        assert_eq!(interrupted.metadata["signaled"], true);

        let read = poll_until_idle(&session_id).await;
        assert_eq!(read["completed_exec"]["status"], "interrupted");
    ...
    async fn shell_session_resize_reports_non_pty_sessions() {
        ...
        let resized = ShellSessionResizeToolPlugin
            ...
            .await
            .unwrap();
        assert_eq!(resized.metadata["resized"], false);
        assert_eq!(resized.metadata["pty"], false);
    ...
    async fn poll_until_idle(session_id: &str) -> Value {
        let mut since_seq = 0_u64;
        for _ in 0..100 {
            let value = manager()
                .session_read(session_id, since_seq, 32_000)
                .await
                .unwrap();
            since_seq = value["seq"].as_u64().unwrap_or(since_seq);
            if value["status"] != "running" {
                return value;
            }
            sleep(Duration::from_millis(100)).await;
        }
        panic!("shell session did not become idle in time");
    }
